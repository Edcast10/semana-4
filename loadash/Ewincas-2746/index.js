...
// entry es cada item
function filterByDesarrollo(entry) {
    // Retorna un (true o false) si el item contiene el tag de Desarrollo
    return _.includes(entry.tags, "Desarrollo")
}

// Cuando recive un verdadero lo filtra
let entriesFilterByDesarrollo = _.filter(entries, filterByDesarrollo)
//console.log(entriesFilterByDesarrollo)

// imprime solo los titulos
let entriesMapBytitulo = _.map(entries, 'titulo')
//console.log(entriesMapBytitulo)

// Organiza los items que sean el mismo autor
let entriesGroupByautor = _.groupBy(entries, 'autor')
//console.log(entriesGroupByautor)

// mapea los titulos de cada autor 
let entriesMapByAutorTitulo = _.mapValues(entriesGroupByautor, function(entry){
    return _.map(entry, 'titulo')
})
//console.log(entriesMapByAutorTitulo)